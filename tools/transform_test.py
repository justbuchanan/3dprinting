...
def wheel():
    h = 5
    w = Part("wheel")(cylinder(r=20, h=h))
    con = {
        name: invert_connector(c)
        for name, c in cylinder_connectors(w.children[0]).items()
    }
    w.con.update(con)
    print(w.con)
    return w
...
a = axle()
model = a + wheel() @ ('bottom', a.con['bottom']) + wheel() @ ('bottom',
                                                               a.con['top'])
