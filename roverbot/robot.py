...
class TreadWheel(Part):
    def __init__(self, r, hole_r, th, **kwargs):
        ...
        self.add(difference()(
            ...
            # center hole
            translate([0, 0, -INC])(cylinder(r=hole_r, h=th + 2 * INC)),

            # indent in the middle of the outer edge to accomodate treads
            translate([0, 0, gap_th / 2])(difference()(
...
def spoked_wheel(r, hole_r, th, **kwargs):
    ...
    slices = [
        donutSlice(innerSize, outerSize, i * 60, i * 60 + 40)
        for i in range(0, 6)
    ]

    return Part("SpokedWheel", **kwargs)(
        difference()(
...
def main_wheel(**kwargs):
    w = Part("MainWheel", **kwargs)(color(pla_color)(spoked_wheel(
        r=wheel_r, hole_r=wheel_hole_r, th=wheel_th)))
    return w
...
def treads(r1, r2, l=20, w=10, th=1, **kwargs):
    ...
    t = Part("Treads",
             **kwargs)(color("black")(linear_extrude(10)(difference()(
...
def chassis():
    ...
    return Part("Chassis")(
        translate([0, wheel_dist / 2 + wheel_r, wheel_r])(
            rotate([0, 90, 0])(
                scale([0.85, 0.85, 1])(
                    union()(
                        # h, l, w
                        roundedBox([
                            wheel_r * 2, wheel_dist + wheel_r * 2,
                            half_wheel_spacing * 2 - body_gap * 2
                        ], 10, True),
                        translate([wheel_r * -0.1, 0, 0])(roundedBox([
                            wheel_r * 2 * 0.8, wheel_dist - wheel_r * 2,
                            (wheel_th + half_wheel_spacing) * 2
                        ], 5, True)))))))
...
def leg_main(th=wheel_th, outer_scale=0.8, cutout_scale=1.2, **kwargs):
    ...
    def main_chunk():
        return union()(
            linear_extrude(wheel_th)(difference()(hull()(
                wheels(scale=outer_scale)))),
            wheels(scale=cutout_scale))

    def sideplate():
        return linear_extrude(leg_sideplate_th)(hull()(
            wheels(scale=outer_scale)))

    def main():
        return union()(
            translate([0, 0, leg_sideplate_th])(main_chunk()),
            translate([0, 0, th + leg_sideplate_th])(sideplate()),

            # posts
            translate([0, 0, th + leg_sideplate_th])
            (rotate([180, 0, 0])(cylinder(r=3, h=20),
                                 translate([leg_wheel_dist, 0, 0])(cylinder(
                                     r=leg_front_axle_r, h=wheel_th + 2)))))
...
def double_wheel(**kwargs):
    ...
    return Part("DoubleWheel", **kwargs)(union()(spoked_wheel(
        r=wheel_r, hole_r=front_wheel_hole_r,
        th=wheel_th), translate([0, 0, wheel_th + tread_gap_x])(spoked_wheel(
            r=wheel_r, hole_r=front_wheel_hole_r, th=wheel_th))))
...
def packbot():
    ...
    bot.con['leg_br'] = bot.con['wheel_fr'].translated(
        [wheel_th + tread_gap_x, 0, 0]).rotated(leg_theta)
    bot.con['leg_bl'] = bot.con['wheel_fl'].translated(
        [-wheel_th - tread_gap_x, 0, 0]).rotated(-leg_theta)

    leg_front_translate = [
        0, leg_wheel_dist * math.cos(deg2rad(leg_theta)),
    ...
    for pos in ['br', 'bl']:
        t = treads(
            wheel_r, leg_front_wheel_r, l=leg_wheel_dist, th=tread_th, tag=pos)
    ...
    def axle():
        return Part("FrontAxle")(rotate([0, 90, 0])(color(pla_color)(cylinder(
            r=front_wheel_hole_r - 1, h=post_len))))

    bot_main += translate([-post_len / 2, wheel_dist + wheel_r,
                           wheel_r])(axle())

    for conname in ['leg_fl', 'leg_fr']:
        # TODO: set pla_color
        bot_main += spoked_wheel(
            r=leg_front_wheel_r, hole_r=wheel_hole_r, th=wheel_th,
            tag=conname) @ bot.con[conname]
    ...
    # raspi
    if False:
        bot_main += translate([0, PiSizeX / 2 + 10, wheel_r * 1.5])(rotate(
            [0, 0, 90])(union()(PiZeroBody(), PiZeroTestPads())))
