...
def box_pattern(
    ...
    x_tooth_len = calculate_tooth_len(target_tooth_len=sheet_thick*2, span_dist=dims[0],even=True)
    y_tooth_len = calculate_tooth_len(target_tooth_len=sheet_thick*2, span_dist=dims[1],even=False)
    z_tooth_len = calculate_tooth_len(target_tooth_len=sheet_thick*2, span_dist=dims[2], even=False)
    ...
    for i in range(4):
        ...
        c = teeth(h=sheet_thick+tooth_extra_ht, tooth_len=t_len, num=int(ll / t_len)-1)

        # cutout for bottom piece
        main_teeth.append(multmatrix(x.tolist())(
            teeth(h=sheet_thick+tooth_extra_ht, tooth_len=t_len, num=int(ll / t_len)-1, startup=True)
        ))
        ...
        plt = union()(
                ...
                translate([ll,-dims[2]])(
                    rotate([0,0,90])(
                        teeth(h=sheet_thick+tooth_extra_ht, tooth_len=z_tooth_len, num=int(dims[2] / z_tooth_len)))),

                # back (vertical)
                rotate([0,0,-90])(
                    teeth(h=sheet_thick+tooth_extra_ht, startup=True,tooth_len=z_tooth_len, num=int(dims[2] / z_tooth_len))),
            )
    ...
    box = union()
    box_main = translate([inset, inset])(square([dims[0] - inset*2, dims[1] - inset*2]))
