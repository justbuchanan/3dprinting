...
# print(os.environ["CQPARTS_SERVER"])
...
class MyObj(cqparts.Part):
    ...
    def make(self):
        ...
        #     .moveTo(*points[0]).polyline(points[1:]).close() \
        #     .extrude(self.width)
        return cadquery.Workplane('XY').circle(4).extrude(self.width)
...
class Abc(cqparts.Part):
    ...
    def make(self):
        ...
        padding = 5
        height=2
        return cq.Workplane("XY").box(20,45,1) \
            .faces(">Z").workplane() \
            .rect(length-padding,height-padding,forConstruction=True) \
            .vertices().hole(4)
...
# x.exporter('stl')('out/out.stl')

MyBlock().exporter('stl')('out.stl')